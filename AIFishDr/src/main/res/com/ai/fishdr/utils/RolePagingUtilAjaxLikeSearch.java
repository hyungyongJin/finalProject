...
public class RolePagingUtilAjaxLikeSearch {
	...
	public RolePagingUtilAjaxLikeSearch(int currentPage, int totalCount,
	...
	private void makePagingHtmls() {
		...
		//이전 <-만들기
		if ((currentPage-1)!=0) {
			pagingHtmls.append("<li><a href='javascript:ajaxSearchLike(" + (currentPage - 1) + ");'>이전</a></li>");
		}
		...
		for (int i = startPage; i <= endPage; i++) {
			if(currentPage == i ){
				...
				pagingHtmls.append("<li><a href='javascript:ajaxSearchLike(" + i + ");'>" + i + "</a></li>");
			}
		}
		//다음
		if (currentPage<totalPage) {
			pagingHtmls.append("<li><a href='javascript:ajaxSearchLike(" + (currentPage + 1) + ");'>다음</a></li>");
		}else{
		}
