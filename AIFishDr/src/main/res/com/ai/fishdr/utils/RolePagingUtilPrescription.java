...
public class RolePagingUtilPrescription {
	...
	public RolePagingUtilPrescription(int currentPage, int totalCount,
...
/*		String params = "";
		...
		*/
		...
		String query = "&search_keycode=" + PrescriptionManagementController.temp_search_keycode + "&search_keyword=" + PrescriptionManagementController.temp_search_keyword;
		
		//이전 <-만들기
		if ((currentPage-1)!=0) {
			pagingHtmls.append("<li><a href = '" + requestURI + "?currentPage=" + (currentPage - 1) + query + "'>이전</a></li>");
		}
		...
		for (int i = startPage; i <= endPage; i++) {
			if(currentPage == i ){
				...
				pagingHtmls.append("<li><a href = '" + requestURI + "?currentPage=" + i + query + "'>"+i+"</a></li>");
			}
		}
		//다음
		if (currentPage<totalPage) {
			pagingHtmls.append("<li><a href = '" + requestURI + "?currentPage=" + (currentPage + 1) + query + "'>다음</a></li>");
		}else{
		}
