...
$(function(){
	...
	changeCnt = function(value){
		$(location).attr('href','${pageContext.request.contextPath}/user/message/messageList.do?commentCurrentPage='
	...
	$('#delMsg').click(function(){
		...
		if (cnt > 0) {
			...
			$.ajax({
				url : '${pageContext.request.contextPath}/user/message/delMsg.do',
				data : data,
				type : 'POST',
				success : function(data){
					if (eval(data.result)) {
					...
					$(location).attr('href','${pageContext.request.contextPath}/user/message/messageList.do?cnt=${param.cnt}');
				}
			})
		}else{
            ...
            <div id="example1_wrapper"
               ...
               <div class="row">
                ...
                <tr role="row">
	                ...
	                <th class="sorting" tabindex="0"  rowspan="1" colspan="1" style="width: 10%;">보낸사람</th>
	                <th class="sorting" tabindex="0"  rowspan="1" colspan="1" style="width: 70%;">내용</th>
	                <th class="sorting" tabindex="0"  rowspan="1" colspan="1" style="width: 10%;">날짜</th>
                </tr>
                </thead>
                <tbody id="msgListBody">
                 <c:forEach items="${ msgList}" var="list"> 
	                <tr role="row" class="odd">
	                  ...
	                  <c:if test="${list.MS_SEND_ID eq 'admin'}">
	                  <td class="sorting"><a style="color: red;" href="#">${list.MEM_NICKNAME }</a></td>
	                  </c:if>
	                  <c:if test="${list.MS_SEND_ID != 'admin'}">
	                  <td class="sorting"><a style="color: green;" href="${pageContext.request.contextPath }/user/fishgram/fishgramYouList.do?mem_id=${list.MS_SEND_ID}">${list.MEM_NICKNAME }</a></td>
	                  </c:if>
	                  <c:if test="${empty list.MS_GET_DATE}">
	                  ...
	                  <c:if test="${!empty list.MS_GET_DATE}">
	                   <c:if test="${fn:length(list.MS_CONTENT)>30 }">
	                 	 <td><a href="#"><button type="button" value="${list.MEM_NICKNAME }(${list.MS_SEND_ID})/*-${list.MS_SEND_DATE }/*-${fn:replace(list.MS_CONTENT,'\"','\'') }/*-${list.MS_SEND_ID}/*-${list.MS_NO }/*-read" class="viewMsg" style="color:grey; background-color: transparent; border: none;">[읽은 쪽지]&nbsp;&nbsp; ${fn:substring(list.MS_CONTENT,0,30) }...</button></a></td>
	        	       	 </c:if>
	        	         <c:if test="${fn:length(list.MS_CONTENT)<30 }">
		               	   <td><a href="#"><button type="button" value="${list.MEM_NICKNAME }(${list.MS_SEND_ID})/*-${list.MS_SEND_DATE }/*-${fn:replace(list.MS_CONTENT,'\"','\'') }/*-${list.MS_SEND_ID}/*-${list.MS_NO }/*-read" class="viewMsg" style=" color:grey; background-color: transparent; border: none;">[읽은 쪽지]&nbsp;&nbsp;${list.MS_CONTENT }</button></a></td>
	                  	 </c:if>
	                  </c:if>
	                  <td>${list.MS_SEND_DATE }</td>
	                </tr>  
                </c:forEach> 
  				</tbody>
                </table>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
               <button id="send" value="" type="button" class="btn btn-default">답장</button>             
          <button onclick="javascript:location.reload();" type="button" class="btn btn-default" data-dismiss="modal">닫기</button>
   </div>
      </div>
    </div>
  </div>
