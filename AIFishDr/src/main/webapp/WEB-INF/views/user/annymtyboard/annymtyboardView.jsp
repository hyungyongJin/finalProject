					...
					<form id="updateForm" method="post">
						<div class="box">
							...
							<div class="box-body pad">
								...
								<div class="dropdown">
									<img src="https://cafe.pstatic.net/levelicon/1/1_1.gif"
										width="11" height="11">
									&nbsp; 
									<span style="font-size: 10pt; color: blue;">
										${viewInfo[0].BOARD_NAME}
									</span>
									<ul class="dropdown-menu">
										<c:if test="${LOGIN_MEMBER.mem_id !=viewInfo[0].BO_WRITER }">
											<li>
												<a href="#">
													<span onclick="goChatModal('${viewInfo[0].MEM_NICKNAME }(${viewInfo[0].BO_WRITER})');">1:1
														채팅하기
													</span>
												</a>
											</li>
											<li>
												<a href="#">
													<span onclick="goModal('${viewInfo[0].MEM_NICKNAME }(${viewInfo[0].BO_WRITER})');">
														쪽지보내기
													</span>
												</a>
											</li>
										...
										<li>
											<a href="${pageContext.request.contextPath }/user/fishgram/fishgramYouList.do?mem_id=${viewInfo[0].BO_WRITER}">
												블로그가기
											</a>
										</li>
									</ul>
								...
								<div id="beforeContent" 
									style="padding: 20px 30px 20px 30px; height: auto; 
								...
								<c:if test="${viewInfo[0].BO_WRITER != 'admin'}">
									...
									<button id="black" type="button" class="btn btn-danger btn-sm">
										신고하기
									</button>
								</c:if>
							</div>
							<div class="show box-body">
								<table>
									<c:forEach items="${viewInfo[1].fileInfo }" var="file">
										<c:if
											test="${! fn:contains(file.file_content_type, 'image') }">
											<tr>
												<td style="width: 100%;">
													<div class="input-group input-group-sm" style="width: 80%;">
														<input type="text" disabled="disabled" name="file_name"
															class="form-control" size="50" value="${file.file_name }">
														<span class="input-group-btn">
															<button type="button" onclick="deleteFile(this.value);"
																value="${file.file_no }"
																style="width: 50px; margin-right: 10px;"
																class="fileDelBtn btn btn-danger btn-sm">
																삭제
															</button>
															<button type="button" value="${file.file_no }"
																style="width: 100px; margin-right: 10px;"
																onclick="javascript:location.href='${pageContext.request.contextPath }/user/annymtyboard/fileDownload.do?file_no=${file.file_no }';"
																class="downBtn btn btn-info btn-sm">
																다운로드
															</button>
														</span>
													</div>
							...
							<div id="form" class="form-group">
								<button type="button" id="createFile" class="btn btn-info"
									style="margin: 10px; width: 10%; height: 30px;">첨부파일
									등록
								</button>
								<br><br>
								<table id="list">
						...
						<div class="show box box-body">
							<c:forEach items="${viewInfo[1].fileInfo }" var="file">
								<c:if test="${fn:split(file.file_content_type,'/')[0] eq 'image' }">
									...
									<div id="${file.file_no }">
										<img src="/image/${file.file_save_name }"
											style="width: 20%; height: 300px; float: left;">
									</div>
								</c:if>
							</c:forEach>
						</div>
						...
						<div class="dataTables_wrapper form-inline dt-bootstrap">
							<c:if test="${viewInfo[0].BOARD_FUNCTION eq '0'}">
								<form id="insForm" method="post"
									action="${pageContext.request.contextPath }/user/annymtyboard/insertComment.do?commentPage=${commentPage}&currentPage=${currentPage}&search=${search}&keyword=${keyword}">
							...
							<button id="return" type="button"
								...
								class="btn btn-info btn-sm bg-purple pull-right">
								목록
							</button>
							<button id="delBtn" type="button" value="${viewInfo[0].BO_NO }"
								style="width: 140px; margin-right: 10px;"
...
$(function(){
	...
	$('#return').click(function(){
		$(location).attr('href','${pageContext.request.contextPath}/user/annymtyboard/annymtyboardList.do?currentPage=${currentPage}&search=${search}&keyword=${keyword}');
	});
	...
	$('#delBtn').click(function(){
		var bo_no = $(this).val();
		$(location).attr('href','${pageContext.request.contextPath}/user/annymtyboard/deleteannymtyboard.do?bo_no='+bo_no);
	});
	...
	$('#finalUpdate').click(function(){
		...
		$.ajax({
			url: '${pageContext.request.contextPath}/user/annymtyboard/updateBoard.do',
			processData: false,
			contentType: false,
			data: formData,
			type: 'POST',
	...
	$('.afCBtn').click(function(){
		...
		$.ajax({
			url: '${pageContext.request.contextPath}/user/annymtyboard/updateComment.do',
	...
	$('#recommend').click(function(){
		...
		$('#yes').click(function(){
			$.ajax({
				url : '${pageContext.request.contextPath}/user/annymtyboard/boardrecommend.do?bo_no=${viewInfo[0].BO_NO }',
				type : 'POST',
				dataType : 'json',
	...
	$('#black').click(function(){
		...
		$('#yes1').click(function(){
			...
			$.ajax({
				url : '${pageContext.request.contextPath}/user/annymtyboard/boardblack.do',
				data : data,
				success : function(result){
					alert("신고가 완료되었습니다.");
					$('#black').attr('disabled','disabled').text('이미 신고완료한 게시물입니다.');
...
function goModal(value){
	...
	var url = "${pageContext.request.contextPath }/user/annymtyboard/messageModal.do?id="+info;
...
function goChatModal(value){
	...
	var info = encodeURIComponent(value);
	var url = "${pageContext.request.contextPath }/user/annymtyboard/chatModal.do?id="+info;
...
function delComment(comment_no){
	$.ajax({
		url : '${pageContext.request.contextPath}/user/annymtyboard/delComment.do?comment_no='+comment_no,
		success : function(data){
...
function deleteFile(value){
	$.ajax({
		url : '${pageContext.request.contextPath}/user/annymtyboard/deleteFile.do?file_no='+value,
		success : function(data){
